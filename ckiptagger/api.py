    ...
    def __call__(
        ...
        for batch in batch_list:
            ...
            if len(sample_list[0][0]) == 0:
                ...
                parital_seq_segment_list, ws_logits, ws_hiddenState = self.model.predict_label_for_a_batch(sample_list)
        ...
        return word_sentence_list, seq_sentence_list[0],ws_logits, ws_hiddenState
    ...
    def __call__(
        ...
        for batch in batch_list:
            ...
            if len(sample_list[0][0]) == 0:
                ...
                parital_pos_segment_list, pos_logits, pos_hiddenState,labelindex_sequence_list ,s_l, o_mask,label_list = self.model.predict_label_for_a_batch(sample_list)
            for partial_index, index in enumerate(index_list):
        ...
        return pos_sentence_list,pos_logits[0], pos_hiddenState, pos_segment_list, list(s_l), segment_list,index_list
...
class NER:
    def __init__(self, data_dir, disable_cuda=True):
        ...
        with tf.Graph().as_default(): #Create a new graph
            model = model_ner.Model(config)
            model.sess = tf.Session()
            model.sess.run(tf.global_variables_initializer()) #return variables_initializer(global_variables())
            saver = tf.train.Saver()
    ...
    def __call__(
        ...
        for batch in batch_list:
            ...
            if len(sample_list[0][0]) == 0:
                ...
                parital_label_sentence_list, labelindex_sequence_list, logits, hiddenState, c_v, w_v,label_predict = self.model.predict_label_for_a_batch(sample_list)
            for partial_index, index in enumerate(index_list):
        ...
        return entity_sentence_list[0], label_sentence_list[0], logits[0], hiddenState,c_v,w_v,label_predict
        #self.model.hiddenState
...
def _normalize_sentence(raw_sentence): #split one by one char
    normal_string_list = []
    normal_to_raw_index = []
    
    for raw_index, raw_string in enumerate(raw_sentence):
        normal_string = unicodedata.normalize("NFKD", raw_string) #UNICODE字串轉換為NFKD格式
