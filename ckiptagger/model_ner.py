...
class Cross_BiLSTM:
    def __init__(self, name="cross-bilstm", input_d=300, hidden_d=100, layers=2):
        ...
        self.f_cell_list = [] #forward
        self.b_cell_list = [] #backward
        for layer in range(layers):
            is_top_cell = (layer==layers-1)
            current_input_d = input_d if layer==0 else hidden_d*2 #if layer==0 return input_d
    ...
    def transform(self, X, L):
        ...
        with tf.variable_scope(self.name):
            for f_cell, b_cell in zip(self.f_cell_list, self.b_cell_list):
                ...
                hidden_order.append(H)
            
        return H, S_fw, S_bw ,hidden_order
# -------- Cross_BiLSTM End -------- 
...
class Model:
    ...
    def create_encoder(self):
        ...
        with tf.variable_scope(self.name):
            ...
            self.cw_h, _, _, self.h_o = self.bilstm.transform(cw_v, self.s_l) # Cross_BiLSTM then transform return H, S_fw, S_bw (cw_h, _, _)
            cw_h = self.cw_h
    ...
    def predict_label_for_a_batch(self, sample_list):
        ...
        #[batch, s_l_max, output_d] (1, 12, 73)
        ...
        return prediction_list,labelindex_sequence_list,logits, hiddenState, c_v[0],w_v[0],self.label_list

def main():
    config = Config() #classConfig 給初始值
    model = Model(config)   
    tf_config = tf.ConfigProto()
    tf_config.gpu_options.per_process_gpu_memory_fraction = 0.45 #限制GPU使用率45%
